...
def create_booking(
    ...
    try:
        raw_start = datetime.fromisoformat(payload.start_time)
        if raw_start.tzinfo is not None:
            start = raw_start.astimezone(timezone.utc).replace(tzinfo=None)
        else:
            start = parse_input_datetime(payload.start_time)

        raw_end = datetime.fromisoformat(payload.end_time)
        if raw_end.tzinfo is not None:
            end = raw_end.astimezone(timezone.utc).replace(tzinfo=None)
        else:
